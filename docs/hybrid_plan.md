...
整个方案采用模块化设计，将数字人驱动分解为两个独立但可同步的模块：

1. **Body 模块**：使用 EMAGE 生成的 SMPL-X 参数（包含 body、hands 和全局平移），保留 EMAGE 在全身手势与肢体连贯性上的最大优势。  
2. **Face Expression 模块**：切换到 LAM_Audio2Expression 生成的 51/52 个标准 ARKit blendshapes 系数，实现直接兼容 Unreal ARKit 且具备原生实时能力。该模块已覆盖 eye 相关表达；在本机真实 LAM 输出中已确认存在 `eyeBlink*`、`eyeLook*`、`eyeSquint*`、`eyeWide*` 等 14 个 eye blendshapes。
...
- 输出要求：每帧输出 51/52 个标准 ARKit blendshapes 系数（0~1 范围），包含 lip-sync、upper face 表情，以及 eye 相关系数。
- 作用：提供直接可用于 Unreal ARKit 的面部表情系数，实现零转换成本和实时驱动；eye/blink 也统一由该模块提供，不再拆出独立 eye 模块。

### 3. 整体执行流程

1. **准备阶段**：确保 EMAGE 已完全跑通，使用同一段测试 wav 文件分别运行 Body 模块和 LAM_Audio2Expression，生成各自独立的输出文件。
2. **推理阶段**：两个模块可并行处理同一段音频（推荐使用多进程方式实现并行推理）。
3. **融合阶段**：编写融合逻辑，按音频总时长和统一帧率（默认 30fps）对齐两个模块的输出参数，处理可能的时序偏移。
4. **输出阶段**：生成标准结果文件夹，包含 body 参数文件和 face ARKit 参数文件。
5. **验证阶段**：使用 Viser 播放 body 参数，使用 Unreal Engine 加载包含 eye 相关系数在内的 ARKit blendshapes 进行端到端视觉验证。

### 4. 融合层核心逻辑要求
融合层需完成以下工作：
- 以音频总时长为基准，将两个模块的输出序列强制对齐到同一帧序列（默认 30fps）。
- 处理可能的帧数差异（通过线性插值或截断实现）。  
- 保留 EMAGE 的 body 参数完整性，仅替换 face 部分。
...
- [ ] 使用同一段短测试 wav 文件分别运行 EMAGE 和 LAM_Audio2Expression，确认两个模块均能正常输出参数文件。
- [ ] 确认 LAM_Audio2Expression 的真实输出中包含 eye 相关 blendshapes，并在 adapter/文档中固定该事实。
- [ ] 实现并行推理逻辑，让两个模块同时处理同一段音频。
- [ ] 实现融合逻辑，按时间戳对齐并合并两个模块的参数。
- [ ] 生成标准结果文件夹并进行 Viser + Unreal 端到端验证。  
- [ ] 测试端到端延迟，目标控制在合理范围内（适合实时对话场景）。  

### 6. 注意事项与风险控制

- **时序对齐**：两个模型默认输出帧率可能存在细微差异，必须严格以音频时长为基准进行对齐。
- **Eye 部分处理**：当前将 eye 视为 `LAM_Audio2Expression` face 输出的一部分统一处理，不再维护独立 eye 模块。
...
- **下游兼容**：最终输出的 ARKit blendshapes 已包含 eye 相关系数，可直接用于 Unreal Engine，body 参数可直接用于 Viser 验证，无需额外转换。
