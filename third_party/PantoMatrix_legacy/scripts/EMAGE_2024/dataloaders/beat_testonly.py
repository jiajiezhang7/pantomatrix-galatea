...
def _serialize_sample(value):
    if hasattr(pyarrow, "serialize"):
        return pyarrow.serialize(value).to_buffer()
    return pickle.dumps(value, protocol=pickle.HIGHEST_PROTOCOL)


def _deserialize_sample(value):
    if hasattr(pyarrow, "deserialize"):
...
class CustomDataset(Dataset):
        ...
        for ratio in self.args.multi_length_training:
            ...
            for i in range(num_subdivision): # cut into around 2s chip, (self npose)
                ...
                if sample_pose.any() != None:
                    ...
                    is_correct_motion = _has_data(sample_pose)
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            sample = txn.get(key)
            sample = _deserialize_sample(sample)
